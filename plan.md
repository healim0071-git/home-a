...
- **해결 및 개선 내역**:
  ...
  2. **자동 FAQ 발행 엔진(`auto_faq_engine.js`) 무제한 연속 발행 구현**:
     - 기본 18개 질문이 모두 등록된 후에도 중단 없이, 다음 회차의 심층 임상 Q&A(`[심층 FAQ {N}문] ...`)를 고유 제목으로 자동 생성하여 19개, 20개, 21개... 무제한 연속 발행되도록 개선.
  3. **권한 판정(`isHealimSuperAdmin`) 안전성 보강**:
     - `u.uid`, `u.id`, `u.grade`, `u.role`을 종합 판정하여 최고관리자의 글쓰기 권한이 100% 안정적으로 유지되도록 강화.
- **검증 결과 (`scratch/verify_unlimited_posts.js`)**:
  - FAQ: 수동 작성(#19) 및 자동 발행(#20) 정상 작동 (`FAQ Unlimited Check: PASS`).
  - 치료후기: 수동 작성(#7) 정상 작동 (`Reviews Unlimited Check: PASS`).
  - 유튜브: 수동 등록(#16) 및 페이지네이션 확장 정상 작동 (`YouTube Unlimited Check: PASS`).
  - 칼럼: 수동 작성(#22) 및 자동 발행(#23) 정상 작동 (`Columns Unlimited Check: PASS`).
  - 정적 빌드 검증 (`hugo --minify`): 에러 0건 통과.
