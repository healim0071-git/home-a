/**
 * Healim Dysautonomia Clinical Column Auto-Publishing Standalone CLI Script
 */
...
console.log('=== Healim Dysautonomia Clinical Column Auto-Publishing CLI Engine ===');
console.log('Total Clinical Articles in Pool: ' + columnsPool.length);

const args = process.argv.slice(2);
if (args.includes('--simulate')) {
  console.log('\n--- 10-Step Schedule Simulation (4~5 posts/week, 08:00~11:00 AM) ---');
  let cur = new Date();
  for (let i = 0; i < 10; i++) {
    ...
    const s = String(cur.getSeconds()).padStart(2, '0');
    const art = columnsPool[i % columnsPool.length];
    console.log(`Post #${i+1}: ${y}.${m}.${d} ${h}:${min}:${s} | Title: ${art.title.slice(0, 35)}...`);
  }
}

if (args.includes('--list')) {
  console.log('\n--- 14 Clinical Column Articles Detail ---');
  columnsPool.forEach((col, idx) => {
    const rawLen = col.content.length;
    console.log(`#${idx+1} [Raw: ${rawLen}자] ${col.title}`);
  });
}
