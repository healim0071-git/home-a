/**
 * auto_publish_faq.js
 * 해아림한의원 자율신경실조증 FAQ 자동 발행 엔진 & 콘텐츠 풀
 * 
 * [요구사항 명세]
 * 1. 대상: 자율신경실조증 및 자율신경계 이상증상 환자 다빈도 질문 (12선 순환)
...
console.log('--- Checking FAQ Content Lengths ---');
autoFaqContentPool.forEach((item, idx) => {
  // Extract text only (exclude image markdown and link urls)
  ...
  console.log(`   - Raw Length: ${item.content.length} chars | Clean Body Length: ${cleanBody.length} chars`);
});
...
function calculateNextScheduleTime(baseDate = new Date()) {
  ...
  // Choose interval: 2 days or 3 days (randomly alternates to achieve 2~3 posts/week)
  const dayOffset = Math.random() < 0.5 ? 2 : 3;
  d.setDate(d.getDate() + dayOffset);

  // Set random time between 08:00:00 and 10:59:59 (between 8 and 11 AM)
