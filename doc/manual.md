...
> [!NOTE]
> **문서 최종 자동 동기화**: 2026. 9. 17. 오후 11:21:32 (KST) | **Git Commit**: `31ca96d` (main) | **상태**: 수정 작업 중
...
- **자율신경 FAQ & 치료칼럼 6대 의료광고 및 표현 원칙 준수 시스템**:
  1. **지양 단어 및 어감 배제**: 근원치료/근본치료, 완치된다, 전문병원, 전문/특화/첨단 수식 남용, 완벽해결, 부작용이 없다 등 배제
  2. **타 병원과의 비교 우위 배제**: 비교 광고 금지 철저 준수
  3. **'재발안된다' 지양**: '재발율이 낮아진다', '재발 위험을 낮춘다' 완곡 표현 적용
  4. **단정적·과장 표현 배제**: 단언컨대, 100%, 결코 등 배제 및 객관적 관리 가능성 기술
  5. **의료광고법 준수 공익적/의학적 정보 구성**: 질환 메커니즘, 생활 관리, 자가 검진 가이드 중심 서술
  6. **'해아림만의' 독점 표현 및 단정·절대 표현 금지**: '반드시 이렇게 해야한다' / '반드시 된다' 지양, 의학적 근거에 기반한 완곡하고 객관적인 표현 사용
- **자율신경 FAQ 무한 자동 발행 엔진**:
  - `scripts/auto_publish_faq.js` 및 `static/js/auto_faq_engine.js` 연동
  - 주 2~3회 자동 스케줄링 발행 및 `즉시 1편 발행` 기능 지원
  - 환자들이 자주 묻는 30대 핵심 임상 질문에 대한 6대 원칙 준수 의학적 답변 제공
- **치료후기 블러 게이트 (Member Lock Gate)**:
  - 의료법 준수를 위한 비회원 블러 처리 및 1:1 카카오/네이버 로그인 모달 연동
- **자율신경실조증 유튜브 갤러리**:
  - 실제 원장단 출연 의학 영상 반응형 임베드
- **자율신경 치료 칼럼 엔진**:
  - `scripts/auto_publish_column.js` 및 `static/js/auto_column_engine.js` 연동
  - 30대 임상 칼럼 풀 기반 6대 원칙 준수 자동 발행 지원
