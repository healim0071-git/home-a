      ...
      text: |
        ...
        var defaultReviewsData = [
          ...
          },
          {
            ...
            "content": "매일 아침 눈을 뜨자마자 뒷목이 뻣뻣하고 관자놀이를 밴드로 꽉 조이는 듯한 두통에 타이레놀과 이지엔을 한 움큼씩 먹으며 버텼습니다. 위장까지 상해 진통제 부작용이 심각해졌지만 약 없이는 하루도 버틸 수 없었습니다.\n\n해아림한의원에서 교감신경 과긴장으로 인한 근막 혈관 수축성 두통이라는 정확한 진단을 받고 치료에 들어갔습니다. 경추 자율신경절의 긴장이 풀리면서 머리를 옥죄던 묵직한 통증이 거짓말처럼 흩어졌습니다. 현재 진통제를 완전히 끊은 지 1년이 넘었지만 단 한 번도 두통이 재발하지 않았습니다. 머리가 맑아지니 매일매일 출근길이 가볍고 행복합니다.",
            "hasTreatment": false,
            "treatmentType": null,
            "isCustom": false,
            "isCanonical": true
          ...
          },
          {
            ...
            "content": "스트레스만 받으면 위장이 쥐어짜듯 뒤틀리며 극심한 경련이 발생해 119 구급차를 탄 것만 10번이 넘습니다. 대학병원 위장관 조영술과 내시경 검사를 매년 반복해도 \"신경성 위경련이니 마음을 편히 가지라\"는 허탈한 답변뿐이었습니다. 지인의 강력한 추천으로 해아림한의원을 찾아 자율신경과 위장관 연동 신경망을 안정시키는 맞춤 치료를 시작했습니다. 복부 담적을 삭히고 간위불화(肝胃不和)를 해소하는 탕약을 복용하자 2주 만에 복부 팽만과 위경련 조짐이 사라졌습니다. 3달 안정 회복 프로그램을 마친 후 지난 10년 동안 단 한 번도 위경련이 재발하지 않았으며, 지금도 감사한 마음으로 건강하게 사업을 운영하고 있습니다.",
        ...
        function renderColumnsList() {
          ...
          pageItems.forEach(function(item, idx) {
            ...
            var mobileAdminBtns = isSuperAdmin ? (
              ...
              '<button type="button" class="px-1.5 py-0.5 text-[11px] text-[#1c6e78] bg-[#eaf3f4] rounded border border-[#badfe3]" onclick="event.stopPropagation(); openEditModal(\'columns\', \'' + safeColId + '\')">수정</button>' +
              '<button type="button" class="px-1.5 py-0.5 text-[11px] text-red-600 bg-red-50 rounded border border-red-200" onclick="event.stopPropagation(); handleDeletePostDirect(\'columns\', \'' + safeColId + '\')">삭제</button>' +
              '</span>'
            ) : '';
