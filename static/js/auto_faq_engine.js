 ...
 * 2. 분량: 질문에 대한 답변 1,000자 내외의 깊이 있는 심도 있는 한방신경정신과 해설
...
가장 먼저 당부드리고 싶은 점은, **현재 복용 중인 양약을 임의로 한 번에 갑자기 중단하는 것은 권장되지 않으며 신중해야 한다는 점**입니다. 신경안정제는 뇌신경 억제 수용체(GABA)에 강제로 결합하여 신경을 가라앉히는 약물이므로, 갑자기 중단할 경우 신경계가 이전보다 훨씬 더 극심하게 흥분하는 **'반동 현상(Rebound Effect)'**이 발생하여 불면, 공황, 심박수 급등이 찾아올 수 있습니다.
...
[전국 지점 안내](https://www.healim.com)`
    },
    {
      ...
      content: `"목과 어깨가 항상 돌덩이처럼 굳어 있고, 뒤통수가 찌릿찌릿 쥐어짜듯 아프면서 귀에서 삐- 하는 이명 소리가 들려요." 자율신경실조증을 앓는 분들 중 80% 이상이 만성적인 경추 긴장과 후두부 통증, 어지럼증, 이명을 함께 호소하십니다. 척추의 구조와 신경계는 밀접하게 유기적으로 연결되어 있습니다.
...
[전국 지점 안내](https://www.healim.com)`
    },
    {
      ...
      summary: '치료 기간과 시소 반응(호전과 악화의 파동) 극복 과정 및 체계적인 항상성 완성 단계',
...
방광의 소변 저장과 배출은 척추 하부(흉추 11번~요추 2번)의 교감신경과 천추(천골 2~4번)에서 뻗어나오는 부교감신경(골반내장신경)의 안정적인 길항 작용으로 이루어집니다. 평상시 소변을 모을 때는 교감신경이 방광 배뇨근을 이완시키고 괄약근을 조여 소변이 새지 않게 합니다. 반대로 소변을 볼 때는 부교감신경이 방광을 쥐어짜고 괄약근을 열어 시원하게 배출시킵니다.
...
[전국 지점 안내](https://www.healim.com)`
    },
    {
      id: 'pool-faq-25',
      category: '복약안전/한약',
      title: '자율신경실조증 한약을 수개월간 장기 복용해도 간이나 신장에 무리가 없나요? 안전성 검증은 어떻게 이루어지나요?',
...
일반적으로 시중에서 건강식품으로 유통되는 출처 불명의 약초나 민간요법 추출물은 중금속, 잔류 농약 검증을 거치지 않아 위해를 일으킬 수 있습니다. 하지만 해아림한의원에서 사용하는 모든 약재는 **식품의약품안전처(KFDA)의 엄격한 hGMP(우수 한약재 제조 및 품질관리 기준)** 엄격한 품질 기준을 통과한 규격품 한약재만을 사용합니다. 중금속, 잔류 농약, 이산화황, 잔류 이산화탄소, 곰팡이 독소 검사에서 완전 적합 판정을 받은 안전한 한약재만을 엄선합니다.

실제로 다수의 국내외 임상 연구와 대한한의학회 추적 관찰 데이터에 따르면, 자율신경실조증으로 한약을 3개월 이상 복용한 환자군에서 간기능(AST, ALT, 감마-GTP) 및 신기능(BUN, 크레아티닌) 수치가 치료 전과 비교하여 정상이거나 오히려 개선되는 결과가 일관되게 입증되었습니다. 자율신경이 회복되면서 간과 신장의 미세 순환 혈류가 증가하고 신체 대사 능력이 정상화되기 때문입니다.
...
또한 자율신경실조증으로 상체에 화열(火熱)이 울체되어 있으면, 음식의 미세한 온도 변화에도 체온 조절 중추인 시상하부가 민감하게 반응하여 과도한 발한 명령을 내리게 됩니다. 시중의 땀 억제제는 얼굴 피부에 바르기 어렵고 일시적일 뿐 신경 신호의 혼선을 바로잡지 못합니다.
...
해아림한의원에서는 과흥분된 근육과 신경을 즉각적으로 이완시키는 대표 처방인 **작약감초탕(芍藥甘草湯), 조등산(釣藤散)**을 처방하여 신경 전달의 과전류를 진정시킵니다. 아울러 상부 경추와 턱관절을 교정하는 추나요법과 안면 미세 침구 치료를 통해 눌려 있던 안면신경 통로를 이완시킴으로써 떨림 증상을 체계적으로 소실시킵니다.
...
[전국 지점 안내](https://www.healim.com)`
    },
    {
      id: 'pool-faq-29',
      category: '수족냉증/레이노',
      title: '한여름에도 에어컨 바람을 쐬면 손가락 끝이 하얗게 질리고 시립니다. 레이노 증후군과 자율신경실조증은 무슨 관계인가요?',
      image: '/images/faq/faq_29_raynaud.svg',
      summary: '말초 세동맥의 비정상적 연축 반응과 알파-교감신경 수용체 과민 차단을 위한 온통경락 치료',
      content: `"날씨가 더운 여름철에도 사무실 에어컨 바람만 쐬면 손가락 끝이 시퍼렇게 질리고 얼음장처럼 차가워집니다. 찬물에 손을 넣으면 손가락이 하얗게 변했다가 파랗게 멍든 것처럼 바뀌고 찌릿찌릿 바늘로 쑤시는 통증이 와요. 레이노 증후군이라는데 안정 회복가 가능한가요?"
...
원래 인체는 추위에 노출되면 체온을 보존하기 위해 교감신경이 말초 혈관을 적당히 수축시킵니다. 그러나 자율신경실조증 환자는 교감신경의 수축 명령이 통제 불능 상태로 폭주합니다. 손가락 끝 세동맥이 완전히 닫혀버리는 극단적인 혈관 연축(Vasospasm)이 발생하여 혈액 공급이 순간적으로 0에 가깝게 차단됩니다. 
...
여기에 자율신경실조증으로 교감신경이 흥분하여 혈압 변동 폭이 커지고 혈관 벽의 긴장도가 높아지면, 혈액이 소용돌이치며 혈관벽을 치는 소리가 귓속 내이로 크게 전도되는 것입니다. 진정제나 혈액순환제 복용만으로는 굳어진 두개골 뼈와 근막의 물리적 압박을 해소할 수 없습니다.
