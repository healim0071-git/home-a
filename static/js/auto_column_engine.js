 ...
 * (자율신경실조증 한방신경정신과 전문의 치료칼럼 자동 발행 엔진)
 * 
 * - 14대 임상 칼럼 풀 순환 발행
...
(function() {
  ...
  // 14 In-depth Clinical Column Articles (~1,600 chars each)
  var columnsPool = [
  {
    "idPrefix": "col-auto-1",
  ...
  function checkAndRunAutoColumnPublish(isManual) {
    ...
    // Determine article to publish from pool
    var poolIdx = (typeof state.poolIndex === 'number') ? state.poolIndex : 0;
    if (poolIdx < 0 || poolIdx >= columnsPool.length) poolIdx = 0;
    var article = columnsPool[poolIdx];
    ...
    var alreadyExists = currentColumns.some(function(item) {
      return item.title === newPost.title;
    });

    if (alreadyExists && !isManual) {
      // Advance to next article without duplicate insertion
      state.poolIndex = (poolIdx + 1) % columnsPool.length;
      state.nextScheduledTime = calculateNextColumnSchedule(new Date()).getTime();
    ...
    if (isManual && typeof window.alert === 'function') {
      alert('⚡ [자율신경 치료칼럼 자동 발행 완료]\n\n' +
            '제목: ' + newPost.title + '\n' +
            '작성자: ' + newPost.author + ' (' + dateStr + ')\n' +
            '분량: 1,600자 내외 임상 칼럼\n' +
            '상태: 썸네일 단일 표시 및 하단 3대 링크 포함 즉시 등록되었습니다.\n\n' +
